...
class Interfaz:
    def __init__(self):
        ...
        self.barra_menu()
        self.v_frame = tk.Frame(self.ventana,width='600', height='250')
        self.v_frame.pack(fill='both', expand='True')
        img = Image.open('int2.png')
        img = img.resize((25,25), Image.ANTIALIAS)
        img = ImageTk.PhotoImage(img)

        # Textos
        self.label_nCliente = tk.Label(self.v_frame, text='Nuevo cliente', font=('Candara', 15)).place(x=235, y=10)
        self.label_edad = tk.Label(self.v_frame, text='Edad:', font=('Candara', 15)).place(x=136, y=70)
        self.label_tempDia = tk.Label(self.v_frame, text='Temperatura del día:', font=('Candara', 15)).place(x=10, y=110)
        self.label_sug = tk.Label(self.v_frame, text='La sugerencia es una bebida:', font=('Candara', 15)).place(x=10, y=170)

        # Entrada de texto
        self.dato_edad = tk.StringVar()
        self.dato_temp = tk.StringVar()
        self.entry_edad = tk.Entry(self.v_frame, textvariable=self.dato_edad, font=('Candara', 12))
        self.entry_edad.place(x=192, y=76)
        self.entry_temp = tk.Entry(self.v_frame, textvariable=self.dato_temp, font=('Candara', 12))
        self.entry_temp.place(x=192, y=115)

        # Botones
        self.boton_reinicio = tk.Button(self.ventana, text = 'Nueva consulta', command = self.nueva_consula, cursor = 'hand2')
        self.boton_reinicio.place(x=495,y=212)
        self.boton_consult = tk.Button(self.ventana, text = 'Consultar', command = self.sugerencia, cursor = 'hand2')
        self.boton_consult.place(x=425,y=212)
        self.boton_info = tk.Button(self.v_frame, width=25, height=25, image= img, command = self.info, cursor = 'hand2').place(x=557, y=5)
    ...
    def sugerencia(self):
        ...
        if self.recomendacion_fria > self.recomendacion_caliente:
            ...
            self.label_resultado = tk.Label(self.v_frame, text = 'FRÍA', font=('Candara', 14), fg='blue')
            self.label_resultado.place(x=260, y=172)
            
        else:
            #print('La sugerencia es una bebida: caliente')
            self.label_resultado = tk.Label(self.v_frame, text = 'CALIENTE', font=('Candara', 14), fg='red')
            self.label_resultado.place(x=260, y=172)
    ...
    def objetivo(self):
        ...
        self.v_objetivo.title('Objetivo del proyecto')
        self.v_objetivo.iconbitmap('im_bebida.ico')
        self.v_objetivo.geometry('550x350')
